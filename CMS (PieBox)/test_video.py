...
def StartVideoPlayerThread():
    try:
        ...
        logging.error("Video player thread quit unexpectedly")
        self.Quit()
...
def videotest3():
    print("Starting video test 3")

    # Start threads
    videothread = threading.Thread(name='videoplayerthread', target=StartVideoPlayerThread)
    videothread.start()

    fileloc1 = "/home/pi/SanddStudios/manatees_logo.png"
    fileloc2 = "/home/pi/SanddStudios/uscenes_h-264_hd_test.mp4"
    fileloc3 = "/home/pi/SanddStudios/yashi.jpg"
    ...
    while True:
        ...
        if gg == 1:
            ...
            _videoLink.SendCmd('{"action":"stop"}')
            _videoLink.SendCmd(json.dumps({VideoPlayer.ACTION : VideoPlayer.PLAY,
                                           VideoPlayer.FILE : fileloc2,
                                           VideoPlayer.SOUND_LEVEL : 5}))
        elif gg == 30:
            _videoLink.SendCmd('{"action":"stop"}')
            _videoLink.SendCmd(json.dumps({VideoPlayer.ACTION : VideoPlayer.PLAY,
                                           VideoPlayer.FILE : fileloc3,
                                           VideoPlayer.SOUND_LEVEL : 5}))
        elif gg == 40:
            print("Sending video quit")
            _videoLink.SendCmd('{"action":"exit"}')
...
def videotest4():
    ...
    fileloc = []
    fileloc.append("/home/pi/SanddStudios/manatees_logo.png")
    fileloc.append("/home/pi/SanddStudios/uscenes_h-264_hd_test.mp4")
    fileloc.append("/home/pi/SanddStudios/yashi.jpg")
    ...
    while True:
        ...
        index = (index + 1) % 3
        print("#################### Sending play cmd " + str(loopguard))
        _videoLink.SendCmd(json.dumps({VideoPlayer.ACTION : VideoPlayer.PLAY,
                                       VideoPlayer.FILE : fileloc[index],
                                       VideoPlayer.SOUND_LEVEL : 5,
                                       VideoPlayer.DWELL : 10.0}))
        msg = _videoLink.CheckEventQ(blocking=True, wait=None)
        if msg != None:
            print(msg)
            event_json = json.loads(msg)
            event = event_json[VideoPlayer.EVENT]
            if event == VideoPlayer.DONE:
                print("Video playing COMPLETE")
                _videoLink.SendCmd('{"action":"hide"}')
            elif event == VideoPlayer.ERROR:
                print("Unrecoverable video error!!")
                break
        if loopguard > 3:
